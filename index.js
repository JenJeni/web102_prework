...
const totalContributions = GAMES_JSON.reduce((sum, currVal) => { return sum + currVal.backers},0);

// Check result
//console.log(totalContributions);

// set the inner HTML using a template literal and toLocaleString method to get a number with commas
contributionsCard.innerHTML = `${totalContributions.toLocaleString("en-US")}`;

// grab the amount raised card, then use reduce() to find the total amount raised
const raisedCard = document.getElementById("total-raised");

const totalPledged = GAMES_JSON.reduce((acc, currVal) => { return acc + currVal.pledged }, 0);

// Check result
//console.log( totalPledged );

// set inner HTML using template literal to display dollar format
...
let numOfGames = 0;
for (var i in GAMES_JSON) { numOfGames++;}
...
const totalUnderfunded = underfundedGames.length;

const fullyFundedGames = GAMES_JSON.filter( game => game.pledged >= game.goal );
const fullyFundedCount = fullyFundedGames.length;
